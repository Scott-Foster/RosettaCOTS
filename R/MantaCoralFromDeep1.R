...
predMantaCoralFromDeep <- function( deep_perc=newDensity, pred_type="PI", intLevel=0.95){
  #deep_perc contains the transomscan data that is to be predicted from.
  ...
  pred.from.samp <- function(xx, type){
    ...
    ys <- NULL
    if( type=="PI")
      ys <- stats::rbeta( length( deep_perc), p*xx["phi"], (1-p)*xx["phi"])
    if( type=="CI")
      ys <- p
    if( is.null( ys))
  ...
  fm.post <- readRDS( system.file("extdata", "samples_MantaCoralFromDeep.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_MantaCoralFromDeep.RDS")
  ...
  colnames( tmptmp) <- c( "deep_perc","MantaFromDeep_coral","coral_lower","coral_median","coral_upper")
