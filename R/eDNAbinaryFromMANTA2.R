...
predeDNAbinFromManta <- function( manta_scars, pred_type="PI", manta.nTows=4, nreps=12, intLevel=0.95){
  ...
  pred.from.samp <- function(xx, type){
    ...
    ys <- NULL
    if( type=="PI")
      ys <- stats::rbinom( n = length( p), prob = p, size = nreps)# / nreps
    if( type=="CI")
      ys <- p
    if( is.null( ys))
  ...
  fm.post <- readRDS( system.file("extdata", "samples_eDNAbinaryFromMANTA.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_eDNAbinaryFromMANTA.RDS")
  ...
  tmptmp <- cbind( manDens, rowMeans( tmp), t( apply( tmp, 1, stats::quantile, prob=limmies)))
  colnames( tmptmp) <- c( "manta_density", "Prob_eDNA", "Prob_lower","Prob_median","Prob_upper")
  if( pred_type=="PI")
