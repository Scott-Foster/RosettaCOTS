...
predeDNAbinFromSALAD <- function( SALAD_density, pred_type="PI", nreps=12, intLevel=0.95){
  ...
  fm.post <- readRDS( system.file("extdata", "samples_eDNAbinaryFromSALAD.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_eDNAbinaryFromSALAD.RDS")
  ...
  tmptmp <- cbind( SALAD_density, rowMeans( tmp), rep( nreps, nrow( tmp)), t( apply( tmp, 1, stats::quantile, prob=limmies)))
  colnames( tmptmp) <- c( "SALAD_density","Prob_eDNA", "nReps","Prob_lower","Prob_median","Prob_upper")
  if( pred_type=="PI")
    tmptmp <- tmptmp / nreps
