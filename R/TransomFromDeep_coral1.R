...
predTransomFromDeep <- function( deep_perc=newDensity, pred_type="PI", intLevel=0.95){
  #deep_perc contains the scanr transect data that is to be predicted from.
  ...
  fm.post <- readRDS( system.file("extdata", "samples_TransomFromDeep.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_TransomFromDeep.RDS")
  ...
  colnames( tmptmp) <- c( "deep_perc","TransomFromDeep_perc","coral_lower","coral_median","coral_upper")
