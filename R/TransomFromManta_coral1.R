...
predTransomFromManta <- function( manta_perc, pred_type="PI", intLevel=0.95){
  #manta_perc contains the numeric imputed values from the manta coral cover. Averaged to the site level.
  ...
  pred.from.samp <- function(xx, type){
    ...
    ys <- NULL
    if( type=="PI")
      ys <- stats::rbeta( length( manta_perc), p*xx["phi"], (1-p)*xx["phi"])
    if( type=="CI")
      ys <- p
    if( is.null( ys))
  ...
  fm.post <- readRDS( system.file("extdata", "samples_TransomFromManta.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_TransomFromManta.RDS")
  ...
  colnames( tmptmp) <- c( "manta_perc","TransomFromManta_coral","coral_lower","coral_median","coral_upper")
