...
predeDNAbinFromCULL <- function( CULL_density, pred_type="PI", nreps=12, intLevel=0.95){
  ...
  fm.post <- readRDS( system.file("extdata", "samples_eDNAbinaryFromCULL.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_eDNAbinaryFromCULL.RDS")
  ...
  tmptmp <- cbind( CULL_density, rowMeans( tmp), rep( nreps, nrow( tmp)), t( apply( tmp, 1, stats::quantile, prob=limmies)))
