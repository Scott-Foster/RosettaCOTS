...
predTransectFromManta <- function( manta_perc, numHard=400, pred_type="PI", intLevel=0.95){
  #manta_perc contains the manta tow site level averages for the numeric hard coral value
  ...
  pred.from.samp <- function(xx, type){
    ...
    ys <- NULL
    if( type=="PI")
      ys <- stats::rbinom( n=length( manta_perc), size=numHard, prob=p)# / numHard
    if( type=="CI")
      ys <- p
    if( is.null( ys))
  ...
  fm.post <- readRDS( system.file("extdata", "samples_TransectsFromManta.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_TransectsFromManta.RDS")
  ...
  tmptmp <- cbind( manta_perc, rowMeans( tmp), t( apply( tmp, 1, stats::quantile, prob=limmies)))
  colnames( tmptmp) <- c( "manta_perc","TransectsFromManta_coral","coral_lower","coral_median","coral_upper")
  if( pred_type=="PI")
