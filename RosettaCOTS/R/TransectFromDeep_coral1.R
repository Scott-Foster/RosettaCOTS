...
predTransectFromDeep <- function( deep_perc, numHard=400, pred_type="PI", intLevel=0.95){
  #deep_perc contains the diver transect data that is to be predicted from.
  ...
  pred.from.samp <- function(xx, type){
    ...
    ys <- NULL
    if( type=="PI")
      ys <- stats::rbinom( n=length( deep_perc), size=numHard, prob=p)
    if( type=="CI")
      ys <- p
    if( is.null( ys))
  ...
  fm.post <- readRDS( system.file("extdata", "samples_TransectsFromDeep.RDS", package="RosettaCOTS"))
#  fm.post <- readRDS( samples_TransectsFromDeep.RDS")
  ...
  tmptmp <- cbind( deep_perc, rowMeans( tmp), t( apply( tmp, 1, stats::quantile, prob=limmies)))
  
  if( pred_type=="PI")
    tmptmp[,-1] <- tmptmp[,-1] / numHard
  
  colnames( tmptmp) <- c( "deep_perc","TransectsFromDeep_coral","coral_lower","coral_median","coral_upper")
