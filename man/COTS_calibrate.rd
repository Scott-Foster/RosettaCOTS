...
\arguments{
\item{from}{A string giving the type of measurement to be converted. Options are: "Manta","eDNA_conc","eDNA_prop","coral_transom","coral_deep","coral_manta", "coral_transect", "SALAD", and "Cull". Capitalisation should not matter, but the presence of underscores will. See Details below.}
...
\item{to.effort}{The amount of effort that is assumed for the prediction. If NULL (default), AND it is needed for prediction, then the average of the CCIP-D2 field trip is inserted (the primary survey). SALAD is a special case here (when predicted from Cull), where to.effort can be either measured in time (for a rate) or area (for a density). For SALAD, the default is area (for density). See argument \code{salad.effort.from.cull} to switch between effort measures. The value of \code{to.effort} for coral_transect type is defaulted to be 400, the goal of the field trip, and represents the number of points on the transect that have been scored.}
...
\item{salad.effort.from.cull = "area"}{A character indicating the units of the to.effort when predicting SALAD. Choices are "area" for density prediction (per m2) and "rate" for rate prediction (per minute). Default is "area". This argument is ignore for all cases, except those with to="SALAD" and from="Cull".}
}
\details{
...
\item{coral_transect}{\code{from.metric} is the number of points on the transect with coral. \code{from.effort} is the number of points on the transect (default is 100 and is a nice round/convenient number)..}
}
