...
\item{SALAD}{This is a method that has a number of possible metrics. So, the one that is best matched to the prediction endpoint is chosen. When predicting eDNA_conc or eDNA_prop: \code{from.metric} is the total number of COTS or Scars recorded, and \code{from.effort} is the area searched in hectars. When predicting Manta: \code{from.metric} is the number of COTS or scars seen and \code{from.effort} is the distance swum in metres. When predicting Cull: \code{from.metric} is the number of COTS seen and \code{from.effort} is the time taken, in minutes. If multiple values are given in either argument, then they are summed to the site level.}
